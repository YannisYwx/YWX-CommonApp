...
import com.ywx.common.ext.setSelect
 ...
 * Description: 表盘
 */
class DialFacesAdapter(data: List<DialFace>, var isShowLabel: Boolean = true) : BaseQuickAdapter<DialFace, BaseViewHolder>(R.layout.home_item_dial_face, data) {
    ...
    override fun convert(helper: BaseViewHolder, item: DialFace?) {
        item?.let {
            if (it.id > 0) {
    ...
    inner class OnWatchFaceSelectListener(var data: List<DialFace>) : OnItemClickListener {
        ...
        override fun onItemClick(adapter: BaseQuickAdapter<*, *>?, view: View?, position: Int) {
            ...
            val event = FragmentEvent(LOCAL_DIAL_FACE_CLICK, position)
            "发送点击事件----------------------- $event".logE()
